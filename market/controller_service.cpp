...
#include "controller_service.h"
    ...
    std::unique_ptr< ::my_controller > pmy_controller(::staff::ServiceFactory::Inst().GetService< ::my_controller >());
...
fetch(&pAvg, avg_target.get_string(), pMarket);
	...
	int COinitResult = pmy_controller->COinit(market_init_price, pstate, market_period, control_mode, psetpoint, base_setpoint, heating_base_setpoint, cooling_base_setpoint);
	return 1;
}
...
TIMESTAMP controller_service::presync(TIMESTAMP t0, TIMESTAMP t1){
	...
	int COpresyncResult = pmy_controller->COpresync(base_setpoint, psetpoint, control_mode);
...
TIMESTAMP controller_service::sync(TIMESTAMP t0, TIMESTAMP t1){
	...
	double COsyncResult = pmy_controller->COsync(load, demand2, pstate, total, target, pAvg2, pStd2, base_setpoint, control_mode, market_current_frameclearing_price);
/*
	bid = (KEY)(lastmkt_id == market->market_id ? lastbid_id : -1);
				lastbid_id = submit_bid(this->pMarket, OBJECTHDR(this), -last_q, last_p, bid);*/
	return TS_NEVER;
}
