/** $Id: solar_service.h,v 1.0 2008/07/18
	Copyright (C) 2008 Battelle Memorial Institute
	@file solar_service.h
	@addtogroup solar_service

 @{  
 **/

#ifndef _solar_service_H
#define _solar_service_H

#include <stdarg.h>
#include "gridlabd.h"


class solar_service : public gld_object
{
private:
...
public:
	...
	//note solar_service panel will always operate under the SUPPLY_DRIVEN generator mode
	...
	enum solar_service_TILT_MODEL {LIUJORDAN=0, SOLPOS=1, PLAYERVAL=2};
	enumeration solar_service_model_tilt;
	enum solar_service_POWER_MODEL {BASEEFFICIENT=0, FLATPLATE=1};
	enumeration solar_service_power_model;
	...
	FUNCTIONADDR calc_solar_service_radiation;	//Function pointer to climate's calculate solar_service radiation in degrees
	...
	TIMESTAMP prevTime;
	double *pRhout;
	double *psolar_serviceD;		//Direct solar_service radiation
	double *psolar_serviceH;		//Horizontal solar_service radiation
	double *psolar_serviceG;		//Global horizontal
